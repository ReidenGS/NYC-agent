...
```bash
cp .env.example .env
docker compose up -d postgres redis mcp-profile profile-agent orchestrator-agent api-gateway
```
...
1. 接入 `housing-agent` + `mcp-sql`，完成只读 SQL 生成、校验、最多 3 次重试、无数据返回 `no_data`。
2. 接入 `safety-agent`、`amenity-agent`、`entertainment-agent`，共享 SQL 生成约束，但 prompt 和表白名单分开。
3. 接入 `transit-agent` 和 `weather-agent` 的固定 MCP 工具调用。
4. 把 `mcp-profile` 从内存存储替换为 Postgres/Redis 持久化，保持工具接口不变。
5. 在 `api-gateway /debug/dependencies` 中继续补齐各 Agent 和 MCP 的健康状态。
